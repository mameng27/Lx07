...
namespace DAL
{
    public class WorkInfo
    {
        ...
        public Model.WorkInfoNo GetModel(int id)
        {
            ...
            {
                string sql = "select workinfo.*,activity.activity.activityName from workin join activity on workinfo.activityId=activity.activityId where workId=@id";
        ...
        public IEnumerable<Model.WorkInfoNo> GetPage(Model.WorkPage page)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                ...
                sql += "select * from a where num between (@pageIndex-1)*@PpageSize+1 and @pageIndex*@pageSize;";
        ...
        public int Add(Model.WorkInfo workinfo)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "insert into workinfo(workId,workName,workPicture,uploadTime,workIntroduction,workVerify,userName,activityId,recommend,recommendTime)"+
                    "values(@workId,@workName,@workPicture,@uploadTime,@workIntroduction,@workVerify,@useName,@activityId,@recommend,@recommendTime);";
                sql += "SELECT @@IDENTITY";
        ...
        public int UpdateImg(Model.WorkInfo workinfo)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                ...
                sql = " update update workinfo set workIntroduction=@workIntroduction,where workId=@workId ";
                return cn.Execute(sql, workinfo);
            }
        }
